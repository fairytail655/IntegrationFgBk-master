...
files_img = loadData_files(path_img,format_img);
...
%     saveImage(bkimg_forbkcues, path_save_bkim, filename);       %
    ...
    displayMatrixImage(i,2,2, imread([path_img changeFileFormat(filename,'jpg')]), showcues, simimg, fgimg);
%    displayMatrixImage(i,2,3,img_cur, bkimg_md, simimg, bkimg_forbkcues, re_grayimg_cur, fgimg )
    [time2_1 time2_2 time2_3]
    %特征提取 背景+前景+补偿 生成线索图像 聚合 前景分割 更新背景和模型 生成用于显示的线索图像 保存中间图像
